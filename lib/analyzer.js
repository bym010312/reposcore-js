...
class RepoAnalyzer {
    ...
    async collectPRsAndIssues() {
        ...
        const validLabels = ['bug', 'enhancement', 'documentation', 'typo']; // 유효한 라벨 정의
            ...
            try {
                while (true) {
                    ...
                    response.forEach(issue => {
                        ...
                        if (EXCLUDE_USERS.includes(login)) return;
                        ...
                        if (issue.pull_request !== undefined) {
                            if (issue.pull_request.merged_at !== null) {
                                ...
                                } else if (labelNames.includes('typo')) {
                                    repoMap.get(login).pullRequests.typo += 1;
                                    if (this.repoPaths.length >= 2) totalMap.get(login).pullRequests.typo += 1;
                                } else if (labelNames.includes('bug') || labelNames.includes('enhancement')) {
                        ...
                        // 이슈 처리
                            ...
                            if (issue.state_reason === 'completed' || issue.state_reason == null || issue.state_reason === 'reopened') {
                                if (labelNames.includes('documentation')) {
                                    repoMap.get(login).issues.doc += 1;
                                    if (this.repoPaths.length >= 2) totalMap.get(login).issues.doc += 1;
                                } else if (labelNames.includes('bug') || labelNames.includes('enhancement')) {
                                    repoMap.get(login).issues.bugAndFeat += 1;
                                    if (this.repoPaths.length >= 2) totalMap.get(login).issues.bugAndFeat += 1;
                                }
                            }
                        }
                    });
